...
router.get("/s/:urlId", async (req, res) => {
  try {
    ...
    return res.status(404).send({ error: "Provided short url not found" });
  }
});
...
router.post("/urls", async (req, res) => {
  ...
  try {
    ...
    const oriURL =
      url.includes("www.") && !url.includes("https://")
        ? `https://${url}`
        : url;
    const shortUrl = process.env.APPURL
      ? `${process.env.APPURL}/s/${urlId}`
      : `http://localhost:3000/s/${urlId}`;
    const url_ = new Url({
      urlId,
      userId: req.user._id,
      url: oriURL,
