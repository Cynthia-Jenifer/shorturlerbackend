...
const shortUrlerMailId = process.env.MAIL_ID;
...
router.get("/verify/:userId", async (req, res) => {
  try {
    ...
    const user = await User.updateOne(
      { _id: mongoose.Types.ObjectId(req.params.userId), verified: false },
      { verified: true }
    );
    if (user.nModified > 0) {
    ...
    const messageQuery = queryString.stringify({
...
router.post("/signup", async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign(
      { userId: user._id },
      process.env.SIGN_SECRET || "hellothisissecret"
    );

    const info = await transporter.sendMail({
      from: `"${appName}" <${shortUrlerMailId}>`, // sender address
      to: `${user.email}`, // list of receivers
      subject: `Welcome to ${appName}, Email Verification`, // Subject line
    ...
    return res
      ...
      .send({
        message:
          "unable to signup or please login if you already have an account with us",
        error: err.message,
      });
  }
});
...
router.post("/signin", async (req, res) => {
  ...
  if (!user) {
    return res
      .status(422)
      .send({ error: "Invalid password or email or not verified" });
  }
...
router.post("/forgotPassword", async (req, res) => {
  ...
  try {
    ...
    if (!user) {
      throw new Error(
        `Email not found or not verfied or don't try to be cheeky!`
      );
    }
    ...
    const token = jwt.sign(
      { userId: user._id },
      process.env.RESET_SECRET || "hellothisissecretreset",
      { expiresIn: 60 * 10 }
    );
    ...
    const info = await transporter.sendMail({
      ...
      subject: `Welcome to ${appName}, Reset Password`, // Subject line
...
router.post("/resetPassword", resetPassAuth, async (req, res) => {
  ...
  try {
    ...
    const updatedUser = await User.updateOne(
      { _id: mongoose.Types.ObjectId(req.user._id), reset: true },
      { password: hashedPassword, reset: false }
    );
    if (updatedUser.nModified == 0) {
      return res
        ...
        .send({
          clientUrl,
          message:
            "Password Reset Attempted more than once or Unauthorised Entry!",
        });
    }
